...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("litedoge"))
        return false;
...
{
    // Convert litedoge:// to litedoge:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("litedoge://"))
    {
        uri.replace(0, 12, "litedoge:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Litedoge.lnk";
}
...
{
    return GetAutostartDir() / "litedoge.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Litedoge-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  litedoge-qt [" + tr("command-line options") + "]                     " + "\n";
